...
for l in range(num_types):
	if any(fac_vol[l][c] > 0 for c in range(num_copies)):
		used_types.append(l)
print("Number of neighbourhoods: "+str(len(neighborhoods)))
print("---------------------------------------------------------------------------")
for s in range(len(neighborhoods)):
	end_model = time.time()
	if s == 0 or s == int(0.25*len(neighborhoods)) or s == int(0.5*len(neighborhoods)) or s == int(0.75*len(neighborhoods)):
		print("Neighbourhood "+str(s)+" / "+str(len(neighborhoods))+": "+str(round(end_model - start_model,2)))
	#################################################################################################################
	try:
		model = cplex.Cplex()
		...
		for j in range(len(neighborhoods[s])):
			...
			all_senses.append("L")
			all_rhs.append(capacity[neighborhoods[s][j]])
		...
		for i in range(num_clients):
			for l in range(len(used_types)):
				for c in range(num_copies):
					for m in range(num_bands):
						if X_var[i][l][c][m] == 1.0:
							cost = cost + demand[i]*production_cost[used_types[l]][m]
		for j in range(len(neighborhoods[s])):
			cost = cost + opening_cost[neighborhoods[s][j]]
		cost = cost + model.solution.get_objective_value()

		if round(cost,2) < round(new_solution,2):
			print("New improved solution:" +str(round(cost,2)))
			new_solution = cost
...
for s in range(len(neighborhoods)):
	...
	try:
		...
		for j in range(len(used_locations)):
			for l in range(len(neighborhoods[s])):
				for n in range(num_copies):
					indices = model.variables.add(obj = [setup_cost[used_locations[j]][neighborhoods[s][l]][n]], 
					      	lb = [0], 
					      	ub = [1],
					      	types = ["B"],
					      	names = [Yjln[j][l][n]])
		...
		for i in range(num_clients):
			for c in range(num_copies):
				for l in range(len(neighborhoods[s])):
					for m in range(num_bands):
						indices = model.variables.add(obj = [production_cost[neighborhoods[s][l]][m]*demand[i]], 
					  	lb = [0], 
					  	ub = [1], 
					  	types = ["B"],
					  	names = [Xicm[i][c][l][m]])
		...
		############ 2.1. Every client has to be assigned. ################################################
		for i in range(num_clients):        
			constraint_1 = cplex.SparsePair(ind = [Xicm[i][c][l][m] for c, l, m in itertools.product(list(range(num_copies)), list(range(len(neighborhoods[s]))), list(range(num_bands)))],
			...
			for l in range(len(neighborhoods[s])):
				for m in range(num_bands):       
					constraint_2 = cplex.SparsePair(ind = [Xicm[i][c][l][m] for i in range(num_clients)] + [Zjcm[j][c][l][m] for j in range(len(used_locations))],
					   val = [demand[i] for i in range(num_clients)] + [-upper_bound[neighborhoods[s][l]][m]] * len(used_locations))
					linear_expressions.append(constraint_2)
					all_senses.append("L")
					all_rhs.append(0.0)
		...
		for c in range(num_copies):
			for l in range(len(neighborhoods[s])):
				for m in range(num_bands):       
					constraint_2 = cplex.SparsePair(ind = [Xicm[i][c][l][m] for i in range(num_clients)] + [Zjcm[j][c][l][m] for j in range(len(used_locations))],
					   val = [demand[i] for i in range(num_clients)] + [-lower_bound[neighborhoods[s][l]][m]] * len(used_locations))
					linear_expressions.append(constraint_2)
					all_senses.append("G")
					all_rhs.append(0.0)
		###################################################################################################
		...
		for l in range(len(neighborhoods[s])):
			for c in range(num_copies):
				...
				all_senses.append("L")
				all_rhs.append(facility_cap[neighborhoods[s][l]])
		...
		for i in range(num_clients):
			for j in range(len(used_locations)):
				if t_var[i][j] == 1.0:
					cost = cost + demand[i]*transport_cost[i][used_locations[j]]
		for j in range(len(used_locations)):
			cost = cost + opening_cost[used_locations[j]]
		cost = cost + model.solution.get_objective_value()
		if round(cost,2) < round(new_solution,2):
			new_solution = cost
			print("New improved solution: "+str(round(new_solution,2)))
			better_solution = True
			for i in range(num_clients):
				for c in range(num_copies):
					for l in range(len(neighborhoods[s])):
						for m in range(num_bands):
							if model.solution.get_values(Xicm[i][c][l][m]) > 0.9:
								assignments[i][0] = neighborhoods[s][l]
								assignments[i][1] = c
				for j in range(len(used_locations)):
					if t_var[i][j] > 0.9:
...
for l in range(len(used_types)):
	for c in range(num_copies):
		...
		linear_expressions.append(constraint_12)
		all_senses.append("L")
		all_rhs.append(facility_cap[used_types[l]])
...
for j in range(len(used_locations)):
	...
	linear_expressions.append(constraint_21)
	all_senses.append("L")
	all_rhs.append(capacity[used_locations[j]])
...
for j in range(len(used_locations)):
	for l in range(len(used_types)):
		for n in range(num_copies):
			if Y_var[j][l][n] == 1.0:
				cost = cost + setup_cost[used_locations[j]][used_types[l]][n]
for j in range(len(used_locations)):
